...
| 9 | `async move` 不要捕获不需要的大对象 | Future 未完成前内存无法释放 |
| 10 | 缓存、队列、批处理缓冲必须有容量边界 | 内存占用被流量决定 |
...
| `async move` 捕获大 `Vec`、JSON 或文件内容 | 任务排队或等待期间持续占用内存 | 只捕获必要字段，或在 `.await` 前 `drop` 大对象 |
| 使用无界 channel / cache | 洪峰时内存无限增长 | 使用有界 channel，并给 cache 设置容量 / TTL / 淘汰策略 |
...
- 版本：v1.2.0
- 更新：2026-07-31
- 变更：补充异步任务捕获大对象、队列和缓存容量边界的内存压力规则。
